...
PLUGIN_META = {
    ...
    "version": "1.0",
    "description": "Swarm implementation using the official langgraph-swarm Python package.",
    "author": "Antigravity"
}
...
class LGSwarmWidget(QWidget):
    ...
    def setup_ui(self):
        ...
        self.enable_checkbox = QCheckBox("Enable Official langgraph-swarm")
        ...
        layout.addWidget(QLabel("Available Agents in the Swarm:"))
...
class LGLibSwarmThread(QThread):
    ...
    def run(self):
        try:
            ...
            if use_rag:
                def query_lightrag(query: str) -> str:
                    ...
                    try:
                        ...
                        rag_model_name = self.mw.ui.rag_model_combo.currentText().strip() if hasattr(self.mw.ui, 'rag_model_combo') else ""
                        
                        url = f"{base_url}/query"
                        payload = {"query": query, "mode": mode, "only_need_context": True, "model": rag_model_name}
            ...
            for a in self.agents:
                ...
                for other_a in self.agents:
                    if other_a['name'] != a['name']:
                        a_tools.append(create_handoff_tool(agent_name=other_a['name'], description=f"Transfer control to {other_a['name']} if they are better suited for the task."))
            ...
            workflow = create_swarm(
                workers,
                default_active_agent=workers[0].name if workers else "agent"
            )
                            ...
                            if msgs:
                                ...
                                for msg in msgs:
                                    if getattr(msg, "type", "") == "ai" and msg.content and isinstance(msg.content, str):
                                        ...
                                        if text.strip():
                                            self.status_update.emit(f"<br><span style='color:#3498db;'><b>[{key}]</b></span><br>{text}<br>", False)
                                        ...
                                        self.status_update.emit(f"\n<span style='color:#e67e22;'><i>[{key} transferred control -> {dst}]</i></span><br>", False)
                                        self.full_response_text += f"\n**[{key} transferred control -> {dst}]**\n"
                                        has_transfer = True
            ...
            if self.cancel_flag:
                ...
                self.status_update.emit("<br><span style='color:#2ecc71;'><b>[Swarm decided task is FINISHED.]</b></span><br>", False)
...
def enable_plugin(main_window):
    if getattr(main_window, '_lgswarm_chat_installed', False):
        return
    main_window._lgswarm_chat_installed = True

    lgswarm_widget = LGSwarmWidget(main_window)
    main_window.ui.tabs.addTab(lgswarm_widget, "Langgraph Swarm")
    main_window.lgswarm_widget = lgswarm_widget

    main_window._original_send_message_lgswarm = main_window.send_message
    
    def lgswarm_send_message():
        gc = main_window.lgswarm_widget
        
        if not gc.is_active or not gc.agents:
            return main_window._original_send_message_lgswarm()
        ...
        main_window.generation_thread = LGLibSwarmThread(main_window, gc.agents, main_window.messages)
        ...
        main_window.generation_thread.model = "Official Swarm"
        main_window.generation_thread.start()

    main_window.send_message = lgswarm_send_message


def disable_plugin(main_window):
    if not getattr(main_window, '_lgswarm_chat_installed', False):
        return

    idx = main_window.ui.tabs.indexOf(main_window.lgswarm_widget)
    if idx != -1:
        main_window.ui.tabs.removeTab(idx)
        
    main_window.lgswarm_widget.deleteLater()
    del main_window.lgswarm_widget

    main_window.send_message = main_window._original_send_message_lgswarm
    del main_window._original_send_message_lgswarm

    main_window._lgswarm_chat_installed = False
